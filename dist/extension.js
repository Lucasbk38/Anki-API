...
// src/log.ts
var log = (...message) => console.log(`[${"AnkiAPI".cyan}]: `, ...message);
var logError = (...message) => console.error(`[${"AnkiAPI".red}]: `, ...message);

// src/api.ts
var requestAnkiAPI = async (action, params = {}) => {
    ...
    if (err && err.cause && err.cause.code === "ECONNREFUSED") {
      const result = await showError(
        ...
        [
          ...
          { name: "Cancel", callback: async () => void 0 }
        ]
      );
      if (result)
        return result;
...
var syncAnki = () => requestAnkiAPI("sync");
var getDecksWithIds = () => requestAnkiAPI("deckNamesAndIds");
var createDeck = (deckName) => requestAnkiAPI("createDeck", { deck: deckName });
var findNotes = (deckName) => requestAnkiAPI("findNotes", { query: `"deck:${deckName}"` });
var getNotesInfo = (notesIds) => requestAnkiAPI("notesInfo", { notes: notesIds });
var getModels = () => requestAnkiAPI("modelNames");
var createNote = (note) => requestAnkiAPI("addNote", { note });
var createNotes = (notes) => requestAnkiAPI("addNotes", { notes });
var updateNote = (card) => requestAnkiAPI("updateNoteFields", { note: card });
var findModel = (modelName) => requestAnkiAPI("findModelsByName", { modelNames: [modelName] }).then(([e]) => e);

// src/debounce.ts
var createDebounce = (delay, cb) => {
  let timeout;
  return (...args) => {
...
var parseField = ({ parseMd }) => (str) => parseMd ? md.render(str.split("<br>").join(import_os.EOL), {}) : str;

// src/index.ts
var configFileName = "ankiconfig.json";
var showCardUpdatedStatus = (card, status) => {
...
var getConfig = async (filePath, ask, editFile) => {
  const parseConfig = async (config2) => {
    if (typeof config2.templateNameAsHeader !== "boolean")
    ...
    if (!models.includes(config2.template)) {
      throw new Error("Unable to automatically update config file");
    }
    return config2;
  };
  const getConfigFromDir = async (dirPath) => {
    const parent = import_path.default.dirname(dirPath);
    if (parent === dirPath)
      return {};
    const parentConfig = await getConfigFromDir(parent);
    if ((await (0, import_promises.readdir)(dirPath)).includes(configFileName)) {
      const configFilePath = import_path.default.join(dirPath, configFileName);
      const buffer = await (0, import_promises.readFile)(configFilePath);
      const configContent = buffer.toString();
      const config2 = JSON.parse(configContent);
      return {
        root: config2?.root ?? parentConfig?.root,
        template: config2?.template ?? parentConfig?.template,
        macros: (config2?.macros ?? []).concat(parentConfig?.macros ?? []),
        templateNameAsHeader: config2?.templateNameAsHeader ?? parentConfig?.templateNameAsHeader ?? true,
        parseMd: config2?.parseMd ?? parentConfig?.parseMd ?? false
      };
    }
    return parentConfig;
  };
  return parseConfig(await getConfigFromDir(import_path.default.dirname(import_path.default.resolve(filePath))));
};
var trimArray = (arr) => {
  const trimStart = (array) => array.reduce((g, c) => g.length === 0 && !c ? g : g.concat([c]), []);
  return trimStart(trimStart(arr).reverse()).reverse();
};
var extendLatex = (content, macros, ankiLatex) => {
  const includedMacros = [];
  if (ankiLatex)
...
var FileHandler = class _FileHandler {
  constructor(fileContent, config2, edit, ask) {
    ...
    this.lines = fileContent.split(import_os2.EOL);
  }
  lines;
  existingDecks = {};
  edits = [];
  ...
  addToLine(line, data) {
    ...
    this.edits.push({
      method: "insert",
      position: new import_vscode.Position(line, this.lines[line].length),
      value: compiledData
    });
  }
  addToEnd(lines) {
    ...
    this.edits.push({
      method: "insert",
      position: new import_vscode.Position(line, this.lines[line].length),
      value: lines.join(import_os2.EOL)
    });
  }
  modifyLineData(line, data) {
    const parsedHeader = parseHeader(this.lines[line]);
    if (!parsedHeader.dataLocation)
      return this.addToLine(line, data);
    const range = new import_vscode.Range(...parsedHeader.dataLocation.map((e) => new import_vscode.Position(line, e)));
    ...
    outer: for (let i = 0; i < content.length; i++) {
      for (const sep of ["$$", "$"])
        if (content.slice(i, i + sep.length) === sep) {
          ...
          const latexContent = extendLatex(content.slice(i + sep.length, j), this.config.macros ?? [], true).replace(/_(?! )/g, "_ ").replace(/(?<! )_/g, " _").replace(/\*(?! )/g, "* ").replace(/(?<! )\*/g, " *");
  ...
  getHTML(lines) {
    ...
    while (i < lines.length) {
      ...
      let escaped = _FileHandler.escapeHtml(this.cleanLatex(content));
  ...
  async bindDeck(fileHeaderName, question) {
  ...
  getAnkiHeaderName(value) {
    return this.config.templateNameAsHeader ? `&lt;h2&gt;${value.trim()}&lt;/h2&gt;` : value;
  }
  async handleCards(deckId) {
    const deckName = Object.entries(this.existingDecks).find(([, id]) => id === deckId)[0];
    if (!deckName)
      throw new Error();
    const cards = [];
    let i = 1;
    const getFields = (card) => Object.fromEntries(fields.map((field, index) => [field, card.fields[index]]));
    ...
    const cardsToCreate = cards.filter((e) => !e.header?.data.id || !existingCardsIds.includes(e.header.data.id));
    if (unmatchedExistingCards.length !== 0) {
      for (const unmatchedCard of unmatchedExistingCards) {
        ...
        if (opt === "Create new header with corresponding content") {
          this.addToEnd([import_os2.EOL, unmatchedCard.fields[fields[0]].value + ` <!--id: ${unmatchedCard.noteId}-->`, "", unmatchedCard.fields[fields[1]].value, ""]);
    ...
    if (cardsToCreate.length > 0) {
      ...
      let ids = await createNotes(notes);
      if (ids === null) {
        ids = notes.map(() => null);
...
var showError = async (message, recoverOptions = []) => {
...
var showNotification = (type, message, options = []) => vscode.window[{ info: "showInformationMessage", error: "showErrorMessage", warning: "showWarningMessage" }[type]](message, ...options);
var activate = async (context) => {
  log('Congratulations, your extension "ankiapi" is now active!');
  const ask = async (questionData) => {
  ...
  const makeEdits = (edits, textEditor = vscode.window.activeTextEditor) => textEditor.edit((editor) => {
  ...
  const editFile = async (file, edit, save = false) => {
    const textEditor = vscode.window.visibleTextEditors.find((e) => e.document.fileName === import_path2.default.resolve(file));
    if (textEditor) {
  ...
  const disposable = vscode.commands.registerTextEditorCommand("ankiapi.syncFile", async () => {
    try {
      ...
      const config2 = await getConfig(activeTextEditor.document.fileName, ask, editFile);
      if (!config2)
        return;
      ...
      if (err instanceof Error) {
        if ("recoverOptions" in err) {
          showError(err.message, err.recoverOptions);
        } else {
  ...
  const configs = {};
  const getParents = (dir) => {
    const parent = import_path2.default.dirname(dir);
    if (dir === parent)
      return [dir];
    return [dir].concat(getParents(parent));
  };
  const addDocToConfig = async (doc) => {
    if (doc.isUntitled || !(doc.fileName.endsWith(".md") || doc.fileName.endsWith(configFileName)))
      return;
    const promises = [];
    for (const dir of getParents(import_path2.default.dirname(doc.fileName))) {
      const configFile = import_path2.default.join(dir, configFileName);
      if (!(0, import_fs.existsSync)(configFile))
        continue;
      promises.push((0, import_promises2.readFile)(configFile).then((e) => JSON.parse(e.toString())).then((content) => configs[dir] = content));
    }
  ...
  return {
    extendMarkdownIt(md2) {
      const mathRulesNames = ["math_block", "math_inline", "math_inline_bare_block", "math_inline_block"];
      log(configs);
      for (const ruleName of mathRulesNames) {
        const baseRule = md2.renderer.rules[ruleName];
        md2.renderer.rules[ruleName] = (tokens, ...args) => {
          const macros = getParents(args[2].currentDocument.fsPath).map((k) => configs[k]).filter((e) => typeof e === "object" && e !== null && "macros" in e).map((e) => e.macros).flat().reduce((g, c) => g.some((e) => e.name === c.name) ? g : g.concat([c]), []);
